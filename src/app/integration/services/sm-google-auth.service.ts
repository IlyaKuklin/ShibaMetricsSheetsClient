...
export class SmGoogleAuthService {
	...
	isUserSignedIn(accountName: string): boolean {
		...
		const data: IGoogleUserData[] = JSON.parse(window.localStorage[SmGoogleAuthService.STORAGE_KEY]);
	...
	signIn(): void {
		this.googleAuthService.getAuth().subscribe((auth) => {
			auth.signIn({ prompt: 'select_account' }).then((response) => {
				...
				if (!window.localStorage[SmGoogleAuthService.STORAGE_KEY]) usersArray = [];
				else usersArray = JSON.parse(window.localStorage[SmGoogleAuthService.STORAGE_KEY]);
				usersArray = usersArray.filter((x) => x.name !== userData.name);
				usersArray.push(userData);

				window.localStorage[SmGoogleAuthService.STORAGE_KEY] = JSON.stringify(usersArray);
